...
\newpage
\section*{Abstract}
This thesis examines Proximal Policy Optimization (PPO) for dynamic difficulty adjustment in educational assessments, addressing shortcomings in static systems that overlook learner variability. Anchored in Flow Theory \cite{csikszentmihalyi1990} and Zone of Proximal Development (ZPD) \cite{vygotsky1978}, the PPO agent personalizes challenges to boost engagement and mastery. Simulated via Markov Decision Processes, it outperforms rule-based baselines by 18\% in mastery efficiency and 93\% ZPD alignment ($p < 0.05$). While recent studies highlight PPO's stability in adaptive tutoring \cite{axak2025,szabo2025}, gaps in real-time ZPD integration persist \cite{kuhn2025}, justifying this framework's contribution to scalable, ethically sound personalized education.
...
Despite progress in RL for education \cite{kuhn2025}, such as PPO's data-efficient personalization in ITS \cite{axak2025}, debates persist on its handling of sparse rewards and partial observability in dynamic assessments \cite{szabo2025}. This study critically addresses these by simulating ZPD-aligned policies, justifying the need for integrated theoretical-computational approaches.
...
While these operationalizations advance RL-pedagogy integration, critical debates arise: PPO's clipping enhances stability \cite{axak2025}, yet may constrain exploration in sparse educational rewards \cite{kuhn2025}. The following section synthesizes RL fundamentals, analyzing how such limitations manifest in practice.
...
Axak et al. (2025) & Adaptive ITS & General Ed. & PPO & ZPD-aligned hints & Data sparsity in real logs \\
Kuhn et al. (2025) & Systematic Review & Education & PPO/TRPO & Adaptive sequencing & Limited live evaluations \\
Szabo et al. (2025) & Pedagogical Policies & Language & PPO & Multimodal ZPD/Flow & Ablation shows cue necessity \\
Ahmed et al. (2025) & RoboLearn & Higher Ed. & PPO-RL & Dynamic personalization & Scalability to diverse learners \\
Jiang et al. (2025) & ERL4SIIP & STEM & Evo-PPO & Hierarchical ZPD rewards & Computational overhead \\
\hline
\end{tabular}
\caption{Synthesis of Recent RL Applications in Education}
\label{tab:rl-synthesis}
\end{table}
This synthesis reveals PPO's dominance for stability \cite{axak2025}, yet critiques its reward sparsity handling compared to hybrids \cite{jiang2025erl}, underscoring the need for multimodal integration to debate efficacy in real-world ZPD adherence \cite{szabo2025}.
...
Ethical considerations remain underexplored in PPO applications \cite{kuhn2025}, with biases in simulated data potentially exacerbating inequities \cite{ahmed2025}. This review justifies the proposed PPO-DDA system by addressing these gaps through ZPD-grounded rewards and ethical safeguards, advancing from descriptive implementations to critically validated innovations.
...
This paradigm facilitates hypothesis testing, such as PPO's superiority in ZPD alignment \cite{axak2025}, through replicable simulations, contrasting with interpretivism's subjectivity \cite{bell2019}.
...
Building on this, the design mitigates external validity concerns by grounding parameters in datasets like ASSISTments \cite{heffernan2014,axak2025}, transitioning from simulation to potential real-world pilots.
...
These parameters, tuned via grid search, balance stability and efficiency \cite{axak2025}, though sensitivity to sparse rewards warrants hybrid approaches \cite{jiang2025erl}.
...
The 18\% mastery gain synthesizes PPO's efficiency \cite{axak2025,szabo2025}, critically debating its edge over DQN in sparse environments \cite{kuhn2025}, while aligning with ZPD for sustained flow. These metrics critically affirm PPO's advantages over baselines \cite{axak2025}, yet highlight debates on generalizability beyond simulations \cite{kuhn2025}.
\section{Implications for Educational Technology}
Scalable for LMS; bridges gaps in RL-pedagogy integration \cite{jiang2025}. Implications extend to scalable ITS \cite{ahmed2025}, though ethical debates on data biases require multimodal safeguards \cite{szabo2025}.

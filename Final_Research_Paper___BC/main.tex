...
\newpage
\section*{Abstract}
This thesis explores the application of Reinforcement Learning (RL) to dynamically adjust the difficulty of assessment questions in real-time. Traditional assessments fail to account for individual learner variability, which may hinder engagement and knowledge acquisition. By simulating a student learning environment, the study developed an RL agent that personalizes difficulty based on learner performance to promote concept mastery. The methodology involved creating a simulated classroom with variable question difficulties, modelling learner behaviour, and benchmarking RL performance against rule-based and static approaches. Outcomes included improved learning efficiency and personalized instructional strategies. This research contributes to adaptive learning technologies by integrating educational theory with machine learning design.

This literature review explored the application of Reinforcement Learning (RL) in educational assessment, with a particular focus on Dynamic Difficulty Adjustment (DDA) for enhancing learner engagement and concept mastery. Grounded in educational psychology theories such as Flow Theory and the Zone of Proximal Development (ZPD), the review examined how RL algorithms, especially Proximal Policy Optimization (PPO), can be used to optimize question difficulty in real time, aligning instructional content with individual learner readiness. It critically evaluated current adaptive learning technologies and highlighted their limitations, particularly those relying on static or rule-based systems that fail to respond to evolving learner states. The review also surveyed recent RL-based implementations in educational contexts, identifying key methodological advances and ongoing challenges, including data demands, interpretability, and ethical considerations around privacy. A comprehensive gap analysis revealed a lack of targeted research on the integration of RL with pedagogical theory for real-time adaptive assessment. This review concluded by arguing for a novel research direction: the design of PPO-driven adaptive assessment systems that bridge theoretical foundations with algorithmic innovation to provide more effective, scalable, and personalized learning experiences.
...
In response to this issue, adaptive learning systems have been introduced to create more personalized educational experiences. Many of these systems rely on predefined rule-based logic to adapt content. Such approaches are often limited in their ability to openly respond to changes in learner performance. Reinforcement Learning (RL) is a machine learning model in which agents learn optimal strategies through feedback and interaction with their environment; they can be a promising alternative.
...
Simulations will use synthetic environments mimicking real educational scenarios, such as quiz-based assessments in STEM subjects. This approach is justified by its prevalence in RL research, where real-world data collection poses challenges like privacy concerns \cite{mandel2014}. For example, the design includes multiple experimental conditions: (1) PPO agent training; (2) baseline comparisons (e.g., rule-based IRT models), and (3) sensitivity analyses for hyperparameters like discount factor ($\gamma$). Literature supports this: \cite{savoie2021} used similar experimental simulations to validate RL for flow-aligned learning, demonstrating improved mastery over static methods. The design's appropriateness lies in its alignment with objectives, enabling hypothesis testing on PPO's stability and scalability while ensuring ethical simulation prior to potential deployment \cite{vanlehn2011}.
...
% TODO: This chapter is missing from supplied documents. Work needed: Describe the actual implementation of the PPO-based DDA system. Guideline: Use Python code snippets (e.g., via listings package); detail setup with OpenAI Gym and Stable Baselines3; include architecture diagrams; explain state/action/reward; add baseline code; provide GitHub link for reproducibility. Aim for 10-15 pages with pseudocode and explanations. Run simulations to generate preliminary data if not done.
...
\subsection{State Representation (Learner Proficiency modelling)}

\subsection{Action Space (Difficulty Adjustment)}

\subsection{Reward Function Design (ZPD and Flow Alignment)}
...
% TODO: This chapter is missing. Work needed: Present simulation results. Guideline: Run experiments (10,000 episodes); include descriptive stats, inferential tests (SciPy); visualizations (Matplotlib exports as figures); comparative tables/graphs; sensitivity analysis. Aim for 10-15 pages with evidence supporting hypotheses.
...
\chapter{Discussion}
% TODO: This chapter is missing (partial in proposal's expected outcomes/ethical/conclusion). Work needed: Interpret results linking to objectives/gaps; discuss implications, limitations, ethics; suggest future work. Guideline: Build on proposal's sections 8-10; reference results from Ch. 5; 8-12 pages.
